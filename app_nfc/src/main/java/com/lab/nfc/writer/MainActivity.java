...
public class MainActivity extends Activity {
    ...
    public void onCreate(Bundle savedState) {
        ...
        mPendingIntent = PendingIntent.getActivity(this, 200, new Intent(this, getClass()), PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        mNFCContent.setOnClickListener(new OnClickListener() {
            @Override
        ...
        mBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                writeMode = true;
                showModeToTitle();
                dialog = new AlertDialog.Builder(MainActivity.this).setTitle("\n請將NFC Tag放到NFC手機裝置感應處!\n\n")
    ...
    public void onResume() {
        ...
        //Toast.makeText(context, getIntent().getAction() + "-onResume()", Toast.LENGTH_SHORT).show();
        // 判斷標簽模式
    ...
    public void onNewIntent(Intent intent) {
        ...
        if (NfcAdapter.ACTION_TAG_DISCOVERED.equals(intent.getAction()) || (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(intent.getAction()))) {
            ...
            //Toast.makeText(context, intent.getAction() + "-onNewIntent() know card " + writeMode, Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(context, intent.getAction() + "-onNewIntent() unknow card", Toast.LENGTH_SHORT).show();
        }
        showModeToTitle();
    }
    ...
    private void showNFCTagContent(Intent intent) {
        ...
        if (rawMsgs != null) {
            ...
            if (value == null || value.length() == 0) {
                Toast.makeText(this, "無標記內容", Toast.LENGTH_SHORT).show();
            }
        ...
        try {
            ...
            if (ndef != null) { // 此標簽已格式化
                ...
                if (!ndef.isWritable()) { // 判斷此卡是否是唯讀 ?
                    Toast.makeText(context, "此標簽 read-only.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                    return false;
                }
                if (ndef.getMaxSize() < size) { // 判斷此卡是否有足夠空間寫入資訊 ?
                    Toast.makeText(context, "標簽容量不足\n標簽容量為: " + ndef.getMaxSize() + " bytes\n寫入訊息容量為: " + size + " bytes.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                    return false;
                }
                // 寫入
                ndef.writeNdefMessage(message);
                Toast.makeText(context, "ＮＦＣ標簽資訊寫入成功(pre-formatted)", Toast.LENGTH_SHORT).show();
                dialog.cancel();
                dialog.dismiss();
                writeMode = false;
                return true;
                ...
                if (format != null) {
                    try {
                        ...
                        Toast.makeText(context, "格式化成功並ＮＦＣ標簽資訊寫入成功", Toast.LENGTH_SHORT).show();
                        dialog.cancel();
                        dialog.dismiss();
                        return true;
                    } catch (IOException e) {
                        Toast.makeText(context, "格式化失敗:" + e.toString() + "(請將NFC卡放穩後再試一次 !)", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();

                        return false;
                    }
                } else {
                    Toast.makeText(context, "此ＮＦＣ標簽並不支援NDEF讀寫格式.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                    return false;
                }
            }
            ...
            if (dialog != null) {
                Toast.makeText(context, "寫入失敗:" + e.toString() + "(請將NFC卡放穩後再試一次 !)", Toast.LENGTH_SHORT).show();
                dialog.dismiss();
            }
        } finally {
            try {
                ...
                Toast.makeText(context, "NFC 關閉失敗:" + e2.toString(), Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private void showModeToTitle() {
        setTitle(writeMode?"NFC 寫入模式":"NFC 讀取模式");
